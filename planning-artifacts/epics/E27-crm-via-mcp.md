# E27: CRM via AgenticSAI MCP — Dynamics 365 + HubSpot

**Sprint:** post-pivot S+3 | **Points:** 34 | **Dependencies:** E04 amendment, E24, E25 | **Milestone:** AgenticSAI Pivot

> **Source:** `sprint-change-proposal-2026-05-12-agenticsai.md`, `prd-amendment-2026-05-12-agenticsai.md` (FR-53), `architecture-amendment-2026-05-12-agenticsai.md` (ADR-020). Implementation overlaps with **E17 amendment** — E27 is the new-build narrative for the same scope. Whichever file owns delivery, the other references it; suggest **E27 is the authoritative source for the post-pivot CRM lifecycle** and E17 amendment is the bridging delta on the original epic record.

## Goal

Deliver bi-directional CRM connectivity for **Microsoft Dynamics 365** and **HubSpot** through AgenticSAI **MCP servers** registered per tenant Project. Each MCP server exposes a stable tool surface (`find_account`, `create_deal`, `update_deal_stage`, `enrich_contact`, `attach_note`) callable by AgenticSAI agents during qualification (lead enrichment in-flight), opportunity-lifecycle transitions (deal stage updates), and on-demand enrichment (user-triggered "enrich this account" from opportunity detail). EU Solicit hosts the OAuth callback and stores tokens Fernet-encrypted in `client.crm_connections`; tokens are injected into MCP-server config at registration via AgenticSAI's secrets API. Pipedrive and Salesforce deferred to post-launch as additional MCP-server registrations.
...
- [ ] OAuth callback flow per provider — EU Solicit-hosted; AgenticSAI never sees OAuth dance
- [ ] On successful OAuth: tokens pushed to AgenticSAI MCP-server secrets; `client.agenticsai_mcp_servers.status='registered'`
...
- [ ] Tier-gate Pro+
- [ ] Per-provider rate-limit state on AgenticSAI side; EU Solicit's circuit-breaker only on the EU Solicit → AgenticSAI path
...
|---|---|---|---|
| **S17.30a** Dynamics 365 MCP server — tool spec authoring | 4 | backend | Author the 5-tool MCP spec; PR-reviewed YAML at `services/integrations-api/config/agenticsai_mcp_specs/dynamics365.yaml` |
...
| **S17.32** OAuth callback hosting + Fernet token vault + AgenticSAI secret push | 5 | backend | OAuth flow; on success, push tokens to AgenticSAI MCP-server secrets |
| **S17.33** Token rotation double-sided | 5 | backend | 6h Beat; refresh OAuth → push to AgenticSAI → verify → update `expires_at` |
...
| **S17.36** Workspace archive → MCP secret deletion | 2 | backend | Revoke tokens at provider + delete AgenticSAI secrets + transition status |
...
- `eusolicit-docs/planning-artifacts/architecture-amendment-2026-05-12-agenticsai.md` § ADR-020
- `eusolicit-docs/planning-artifacts/prd-amendment-2026-05-12-agenticsai.md` § FR-53
