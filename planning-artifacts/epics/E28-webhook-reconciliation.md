...
**Sprint:** post-pivot S+1..S+2 (parallel-trackable with E04 amendment) | **Points:** 21 | **Dependencies:** E04 amendment (S04.21 schema, S04.25 receiver, S04.26 reconciler scaffold) | **Milestone:** AgenticSAI Pivot

> **Source:** `sprint-change-proposal-2026-05-12-agenticsai.md`, `prd-amendment-2026-05-12-agenticsai.md` (FR-54, FR-55, NFR-26), `architecture-amendment-2026-05-12-agenticsai.md` (ADR-018).

## Goal

Make AgenticSAI → EU Solicit event delivery reliable, idempotent, and self-healing. Manage Standard Webhooks subscriptions in code (no manual AgenticSAI dashboard config); verify HMAC signatures with `hmac.compare_digest()`; deduplicate via a Redis-backed idempotency cache (7-day TTL); route events to internal Redis Streams for downstream consumers (`data-pipeline`, `client-api`, `notification`, `gateway`); converge non-terminal `gateway.workflow_runs` rows via a 5-minute reconciler that polls `GET /jobs/{jobId}/status`; quarantine poison events in a dead-letter queue with admin alerting.

The **reconciler is authoritative truth** for run lifecycle; webhooks are latency optimisation. This invariant matters because AgenticSAI is the second critical external dependency (after Stripe) and the platform cannot accept silent run loss.
...
- [ ] Webhook receiver endpoint `POST /webhooks/agenticsai` on `agenticsai-gateway` public ingress (TLS, authenticated only by HMAC signature):
  ...
  - [ ] Idempotency cache: Redis SETNX on `webhook_event:{agenticsai_event_id}` with 7-day TTL; duplicate → 200 (already processed)
...
- [ ] 5-minute run-state reconciler (Celery Beat): scans `gateway.workflow_runs` partial index for non-terminal rows older than 30s, polls AgenticSAI `GET /jobs/{jobId}/status`, converges status. Wins race against webhook (idempotent updates via `UPDATE ... WHERE status IN ('pending','running')` guard)
- [ ] Reconciler observability: Prometheus gauges `agenticsai_workflow_runs_nonterminal_total` (by `run_type`), `agenticsai_webhook_dlq_depth_total`, `agenticsai_reconciler_polls_per_minute`, histogram `agenticsai_run_to_terminal_seconds`
- [ ] Tenant-visible degraded-mode banner: on circuit-breaker open >5 minutes against AgenticSAI, surface a status banner via existing notification channel — clears on recovery
...
- [ ] Cross-tenant negative: AgenticSAI event for tenant A's Project routes only to tenant-A-scoped consumers; tenant B can't subscribe to A's events
...
CREATE TABLE gateway.webhook_dlq (
    id                          UUID PRIMARY KEY,
    agenticsai_event_id            TEXT NOT NULL,
    agenticsai_event_type          TEXT NOT NULL,
...
- Rotation failure (e.g. AgenticSAI rejects new subscription) does not deregister old subscription
...
- Malformed payload → row in `webhook_dlq`, no Stream publication, 202 (acknowledge receipt to prevent AgenticSAI retry storm)
- Rate-limited inbound (extremely unlikely but defensive): if Redis SETNX fails (Redis down), fail-closed 503 to encourage AgenticSAI retry rather than risk dropped event
...
Map AgenticSAI event types to internal Redis Streams per architecture amendment §5.3:
- `agenticsai.workflow.completed` → `data-pipeline.opportunity_completion` (E26 consumer) + `client-api.user_notifications` (notification surface)
- `agenticsai.agent.run.completed` → `gateway.workflow_runs_converge` (S04.26 reconciler input) + `notification.run_completion_alerts`
- `agenticsai.kb.file.processed` → `client-api.kb_file_processed` (E25 S25.03 consumer)
- `agenticsai.policy.violation` → `notification.policy_alerts` + `shared.audit_log`
...
Builds on E04 S04.26 (which delivered the basic 5-min reconciler). Hardenings: skip rows where `last_polled_at < now() - 30s` (prevent thundering herd on staggered deploys); exponential backoff on AgenticSAI rate-limit (back off to 10-min cadence per affected tenant on 429); export histogram `agenticsai_run_to_terminal_seconds` and gauges named above. Alert: PagerDuty (per ADR-010 — email + Telegram for current launch posture) on `agenticsai_workflow_runs_nonterminal_total > threshold` sustained for 30 minutes.

**Acceptance:**
- Reconciler completes scan within 5-minute cadence at 10K non-terminal rows (load test)
- Rate-limit handling: 429 from AgenticSAI → backoff applied; metric `agenticsai_reconciler_rate_limit_backoff_active` set
...
- Webhook delivery from EU Solicit → AgenticSAI (one-way from AgenticSAI to us for now; if AgenticSAI ever needs us to push events, separate epic).
