...
**Sprint:** post-pivot S+2 | **Points:** 34 | **Dependencies:** E24 | **Milestone:** SirmaAI Pivot

> **Source:** `sprint-change-proposal-2026-05-12-sirmaai.md`, `prd-amendment-2026-05-12-sirmaai.md` (FR-49 to FR-52), `architecture-amendment-2026-05-12-sirmaai.md` (ADR-019).

## Goal

Make the per-Project SirmaAI knowledge base a first-class, user-managed surface in EU Solicit. Users upload tenders, ESPD templates, company profile documents, past proposals, and qualification rubrics to their company's KB; agents (qualification, quantification, ESPD auto-fill, grant tools, proposal drafter) ground their outputs in retrieved KB passages with citations; users semantic-search across their KB and inspect the documents agents grounded against. Per ADR-019, **SirmaAI is canonical for unstructured artefacts** — EU Solicit holds metadata pointers (`sirmaai_storage_resource_id`, `sirmaai_file_id`, `sha256`) but not the artefact bodies after upload. Tier-gated quotas enforced. Profile updates trigger automatic re-index.
...
- [ ] Upload flow: stream file from `client-api` to SirmaAI `POST /storage-resources/{vectorStoreId}/files` (multipart proxy, no buffering); record `client.sirmaai_kb_files` row with SHA-256 + metadata; emit `kb.file.uploaded` event
- [ ] SirmaAI `storage.file.processed` webhook updates `client.sirmaai_kb_files.parsed_text_available_at`; UI badge transitions from "Processing…" to "Ready"
- [ ] Semantic search endpoint `POST /api/v1/kb/search` wraps SirmaAI `POST /storage-resources/{id}/search`; returns ranked passages with file metadata, source file ID, and parsed-text excerpt
- [ ] Parsed-text download proxy `GET /api/v1/kb/files/{file_id}/parsed-text` wraps SirmaAI `/files/{fileId}/parsed-text/download`
- [ ] Signed-URL artefact download proxy `GET /api/v1/kb/files/{file_id}/download` wraps SirmaAI `/files/{fileId}/download` with EU Solicit auth check
- [ ] Replace flow: replacing a file deletes the old SirmaAI file, uploads new, preserves `client.sirmaai_kb_files` row UUID + tags
- [ ] Archive flow: soft-delete in `client.sirmaai_kb_files` + delete in SirmaAI storage-resource; user can restore within 30 days (re-upload from local backup if needed — bodies aren't kept in EU Solicit)
...
- [ ] Agent grounding: all qualification, quantification, ESPD, grant tools, and proposal drafter agents have read access to their tenant's KB (already in SirmaAI Project scope per ADR-018); structured agent outputs include source-citation references back to `client.sirmaai_kb_files.id`
- [ ] Nightly SHA-256 reconciliation: hash on file in `client.sirmaai_kb_files.sha256` compared against SirmaAI inventory metadata; mismatch → admin alert + audit entry
...
### S25.02: Upload proxy + SirmaAI storage-resource integration
**Points:** 5 | **Type:** backend

Endpoint `POST /api/v1/kb/files` (multipart). Streaming proxy to SirmaAI without buffering the body in memory (FastAPI streaming + httpx streaming upload). On SirmaAI success: persist `client.sirmaai_kb_files` row with SHA-256 computed during stream (incremental hash). Emit `kb.file.uploaded` event. Validate `artefact_category` + content-type before forwarding. Tier-gate Depends checks total-size + per-file size.
...
Webhook handler (in `data-pipeline` or `sirmaai-gateway` consumer per E28 routing): on `sirmaai.kb.file.processed` event, look up `client.sirmaai_kb_files` row by `sirmaai_file_id`, set `parsed_text_available_at = now()`. Frontend KB list page subscribes to TanStack Query invalidation; row state badge transitions `Processing… → Ready`. Idempotent.

**Acceptance:**
- Webhook delivery → UI state update within 5s p95 in dev
- Duplicate webhook → no-op
- Test: file uploaded; SirmaAI webhook delayed; reconciler (per E28) catches missing processed state and converges
...
Three endpoints, all wrapping `sirmaai-gateway` calls with EU Solicit auth + cross-tenant scope check:
- `POST /api/v1/kb/search` → `POST .../storage-resources/{id}/search`; returns ranked passages
- `GET /api/v1/kb/files/{file_id}/parsed-text` → SirmaAI parsed-text download (streaming response)
- `GET /api/v1/kb/files/{file_id}/download` → SirmaAI file download (streaming response with original filename)
...
Replace endpoint `PUT /api/v1/kb/files/{file_id}`: deletes old SirmaAI file, uploads new, preserves the EU Solicit row UUID + tags + category. Archive `DELETE /api/v1/kb/files/{file_id}`: soft-delete in EU Solicit + delete in SirmaAI; sets `archived_at`. Restore `POST /api/v1/kb/files/{file_id}/restore`: only valid within 30 days of archive, re-upload required from local source (UI prompts user — body isn't kept).

**Acceptance:**
- Replace preserves row UUID so historical links don't break
- Archive deletes SirmaAI side within 10s
...
- Subsequent agent runs ground against new profile (test by mocking SirmaAI agent that echoes its retrieved KB passages)
...
Celery Beat (daily 04:00 UTC): for each company's active KB files, query SirmaAI metadata, compare hash. Mismatch → audit log entry + admin alert via existing notification surface. Reconciliation result exported as Prometheus gauge `sirmaai_kb_hash_mismatches_total`.

**Acceptance:**
- 7-day rolling test: synthetic file insertion + SirmaAI-side modification triggers alert
...
Update agent response handling in `sirmaai-gateway`: when an agent run includes `kb_citations` in its structured output (SirmaAI returns citation references for grounded responses), enrich them with `client.sirmaai_kb_files.id` + filename for UI rendering. Update qualification + quantification + ESPD endpoints to surface citations to the frontend. Integration test: agent returns 3 citations; UI shows 3 source file references with click-through to parsed-text view.

**Acceptance:**
- Citation enrichment adds EU Solicit-side metadata (filename, category, archived_at) to every SirmaAI citation
...
- Vector embedding inspection / model selection for the KB — SirmaAI manages this; EU Solicit doesn't expose it.
