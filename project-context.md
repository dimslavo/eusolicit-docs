...
date: '2026-05-25'
last_updated_by: 'retrospective-epic-11-supplementary-2026-05-25'
last_updates:
  - '2026-05-25: Epic 11 supplementary retrospective — 8 new patterns (E11-P01..P08) and 9 new anti-patterns (E11-AP01..AP09) added. Covers stories S11.01–S11.16 + S11.23 (KB-grounded tests). Key findings: agent integration pattern frozen (E11-P01), null-vs-empty-list API design (E11-P02), migration downgrade multi-row safety (E11-P03), module-level arithmetic tolerance (E11-P04), design system component ATDD source-inspection (E11-P05), 503 agent error body assertion template (E11-P06), hook reuse before duplication (E11-P07), KB-grounded agent integration test pattern (E11-P08). Anti-patterns: Done=Stub story completion (E11-AP01 CRITICAL), hardcoded strings in fix passes (E11-AP02), design system violations not caught at ATDD (E11-AP03), TEA reviews in separate stories 9th consecutive epoch (E11-AP04 CRITICAL), retry bypasses form validation (E11-AP05), story status header staleness (E11-AP06), open patches at approval (E11-AP07), null JSONB patch undefined (E11-AP08), NFR blockers surviving story review (E11-AP09 CRITICAL). NFR FAIL: ESPD XML namespace non-conformance + DOCX/PDF event-loop blocking. TRACE_GATE FAIL: P1 75% (S11.07 Logframe+ReportingTemplate backend not implemented). Supplementary retro: eusolicit-docs/implementation-artifacts/epic-11-retro-2026-05-25.md.'
...
53. **(E11) Idempotency for Mutable Operations:** For `POST` or `PUT` operations that are conceptually idempotent (e.g., assigning a framework that's already assigned), the endpoint should handle re-submission gracefully (e.g., by silently skipping the duplicate operation and returning a 200 or 201) rather than returning a conflict error.
...
- **Agent integration pattern is complete, stable, and frozen.** (E11-P01) All future agent integrations MUST use: `AiGatewayClient.run_agent(agent_name, payload)`, flat `{"message": "...", "code": "AGENT_UNAVAILABLE"}` 503 body via JSONResponse (NOT `{"detail": "..."}` HTTPException body), `X-Caller-Service` header, `aigw_timeout_seconds` from settings, no client-side retry. A deviation — different error body shape, client-side retry, missing X-Caller-Service header, direct AIGW URL construction — is a blocking review finding. S11.04 (Grant Eligibility Agent Integration) is the canonical backend reference.

- **`null` vs `[]` is a first-class API design choice for optional list response fields.** (E11-P02) For any response field that is a list AND whose absence carries different UI semantics than an empty list, the field type MUST be `list | None` (not `list`). `null` = agent/service didn't produce data (frontend suppresses the component entirely); `[]` = produced but empty (frontend shows the empty-state component). This distinction must be explicit in: (1) the AC spec, (2) the Pydantic schema, (3) the parser function (defensive handling for both `None` and `[]`), (4) the frontend conditional render. Reference: S11.07 `gantt_data: null` vs `gantt_data: []` parser.

- **Migration downgrade safety requires multi-row data guards.** (E11-P03) Any migration downgrade that recreates a UNIQUE or PRIMARY KEY constraint must include a regression test inserting ≥2 rows with the same grouping key before running the downgrade, then asserting the downgrade completes without constraint violations. Unit tests with single rows per grouping key will pass; production data with multiple rows will fail — this class of bug is only caught with multi-row test data. Pattern: `ROW_NUMBER() OVER (PARTITION BY company_id ORDER BY created_at)` assigns unique version numbers before re-adding the constraint. Reference: S11.01 `downgrade 009→008`.

- **Module-level arithmetic tolerance constant for float comparisons in financial calculations.** (E11-P04) For any endpoint performing float arithmetic on monetary values (EU grant budgets, pricing, billing), define the tolerance as a named module-level constant: `_ARITHMETIC_TOLERANCE = 0.01` (EUR). This makes the tolerance visible at the top of the module, testable in isolation, and discoverable via grep. EU grant budgets don't go below half-euro in practice, so 1-cent tolerance avoids float precision failures without hiding real arithmetic bugs.

- **Design system component ATDD source-inspection is mandatory.** (E11-P05) Any AC specifying a component from `@eusolicit/ui` (e.g., `<Select>`, `<Dialog>`, `<Sheet>`, `<Tabs>`, `<DatePicker>`, `<Combobox>`) MUST have a corresponding ATDD source-inspection assertion in the RED-phase test: `assert "<Select" in component_source and "from @eusolicit/ui" in component_source` (or equivalent import-path check). This catches design-system violations at RED phase — before the developer writes the component — rather than in review. Add to frontend ATDD checklist template for all stories specifying `@eusolicit/ui` components.

- **503 agent error body assertion template.** (E11-P06) All agent integration tests asserting agent-unavailable (503) responses MUST use this template: `assert "message" in data and "code" in data and "detail" not in data`. Include a comment quoting the exact AC-specified JSON shape (e.g., `# AC4: {"message": "...", "code": "AGENT_UNAVAILABLE"}`). The `"detail"` key is the FastAPI HTTPException payload key — flat JSONResponse bodies from agent error handlers never use it. Any test asserting `data["detail"]` is testing the wrong body structure.

- **Hook reuse before duplication in same-domain frontend stories.** (E11-P07) Frontend stories implementing a second panel or view within the same feature domain MUST audit TanStack Query hooks defined in the prior domain story before creating new ones. Document reused hooks explicitly in Dev Notes with the source story reference. Reusing `useComplianceFrameworks` and `useComplianceFramework` from S11.14 directly in S11.15 (without modification) produced a first-pass APPROVED story with zero blocking items.

- **KB-grounded agent integration test pattern with `aigw_env_setup` fixture.** (E11-P08) Integration tests for KB-grounded agent endpoints require: (1) `aigw_env_setup` session-scoped autouse fixture setting `CLIENT_API_AIGW_BASE_URL` to a respx-interceptable URL; (2) `kb_grounded_test_data` fixture creating Company + User + SirmaAIProject + SirmaAIKbFile; (3) `respx.mock` on `{aigw_base_url}/agents/{agent_name}/run`; (4) assertions verifying endpoint output reflects grounded content from mocked agent response. Debug: `SirmaAIProject` has no `project_name` column; `ESPDProfile.espd_data={}` triggers 422 (guard: `if not profile.espd_data`). Reference: `tests/integration/test_kb_grounded_agents.py` (S11.23).
...
- **Don't mark a story `done` when the frontend uses stub API constants for that story's endpoints.** (E11-AP01) A story marked `done` with frontend panels referencing `STUB_LOGFRAME_RESPONSE`, `STUB_REPORT_RESPONSE`, or any similar stub constant IS NOT DONE. TRACE_GATE will correctly fail — that's the system working. The done gate must verify: (a) no stub API function references in frontend for story-owned endpoints, AND (b) TRACE_GATE coverage shows ≥1 GREEN test per covered AC. S11.07 was marked `done` in sprint-status without meeting either condition (SEVERITY: critical).

- **Don't add display strings in TSX during fix passes without routing them through `t()`.** (E11-AP02) The pattern: i18n keys added for AC-specified strings during initial implementation, but strings added during review fix passes (error messages, toast notifications, placeholders) use literal English. This repeated across S11.11, S11.13, S11.14 — three stories, same epic. Three review rounds on S11.13 alone. The structural fix is a `no-hardcoded-strings` ESLint rule that makes literal JSX text a lint error, not a review finding. Add to `apps/client` and `apps/admin` package.json (SEVERITY: high).

- **Don't use native HTML elements when the AC specifies a `@eusolicit/ui` design system component.** (E11-AP03) S11.12 used `<select>` (HTML native) instead of `<Select>` from `@eusolicit/ui` despite AC7 and AC11 explicitly specifying the design system component. Native `<select>` doesn't support the Radix controlled-value pattern; switching requires different form wiring. This is domain knowledge the reviewer must supply — too late. ATDD source-inspection assertions (E11-P05) are the structural prevention. Without them, this class of violation reaches review on every frontend story specifying `@eusolicit/ui` components (SEVERITY: high).

- **Don't put TEA reviews in a separate injected story — they get pre-empted for 9 consecutive epics.** (E11-AP04) `inj-03-tea-review-backlog` has been at `ready-for-dev` since E08. The separate-story approach has 0% success rate across 9 epics. TEA review must be embedded as a mandatory AC in each story's `done` transition: "TEA review executed; score ≥ 80/100 documented in Dev Agent Record; or blocking findings enumerated with remediation plan." Retire `inj-03`. This is a structural intervention, not a cultural one (SEVERITY: critical).

- **Don't call `mutation.mutate(form.getValues())` directly on retry buttons.** (E11-AP05) Three panels in S11.12 had retry buttons calling `mutation.mutate()` directly using `form.getValues()`, bypassing Zod validation entirely. On retry after a failed submission, this allows sending invalid data (e.g., empty `required_capabilities: []`). Retry MUST always route through `form.handleSubmit()` or an explicit pre-mutate validation step. Add to ATDD checklist template: `assert RetryButton calls form.handleSubmit(), not mutation.mutate() directly` (SEVERITY: high).

- **Don't leave story file `Status:` headers stale after sprint-status.yaml is updated.** (E11-AP06) 6/16 E11 story files had stale `Status:` headers at retro time (5th recurrence of this anti-pattern: E09, E12, E13, E15, E11). The story file `Status:` header is the source of truth for NFR assessment, traceability tools, and review agents. A stale `review` or `approved` header causes downstream tools to report false blocking findings and misleads agents reading the story context. Update story file `Status: done` as the FIRST action when sprint-status.yaml is updated (SEVERITY: medium).

- **Don't approve a story with open `[Patch]` items unless they are formally tracked for follow-up.** (E11-AP07) S11.10 was approved with 2 open `[Patch]` items: (1) no test asserts agent payload body (sources list, ISO 8601 timestamp); (2) `PATCH /platform-settings/:key` with JSON `null` for value hits DB NOT NULL → unhandled 500. Open patches at approval must be automatically added to the next epic's injected story queue or tracked as a named follow-up story — never left as silent debt (SEVERITY: medium).

- **Don't leave nullable JSONB list field patch semantics undefined in admin CRUD story ACs.** (E11-AP08) S11.08 returned 500 IntegrityError when `PATCH /frameworks/:id` received `{"rules": null}` because the null guard was missing on the rules list comprehension. The AC did not specify what `null` should mean. Admin CRUD story templates must include explicit null-value AC variants for all nullable JSONB patch fields: "`PATCH` with `{"field": null}` → field cleared, returns `[]`" — or alternatively, "`null` → 422 validation error." The choice is explicit in the AC, not left to implementation judgment (SEVERITY: medium).

- **Don't let NFR blockers survive story-level code review.** (E11-AP09) Both NFR blockers identified in the E11 NFR report (ESPD XML custom namespace, DOCX/PDF event-loop blocking) were present in the initial implementation and survived multiple review rounds. The E07 rule requiring `run_in_executor` for CPU-bound operations (Rule 47 / Critical Rule 49) was not applied to `reportlab doc.build()` or `python-docx Document.save()`. Review agents and ATDD checklists for any story generating DOCX/PDF/XML output MUST include: (1) explicit `run_in_executor` assertion; (2) explicit namespace-conformance assertion for standards-specified XML formats. "Export endpoint" is a trigger term that must activate these checklist items (SEVERITY: critical).
